...
function bindRowEvents(row) {
    ...
    // On medicine change: fetch current rate via AJAX (editable for purchase)
    medSelect.addEventListener('change', function () {
        ...
        fetch('/Purchase/GetMedicineInfo/' + medId)
            .then(r => r.json())
            .then(data => {
                rateInput.value = parseFloat(data.rate).toFixed(2);
                medSelect.title = 'Current stock: ' + data.stock;
                calcLine(row);
            })
            .catch(() => alert('Could not fetch medicine info.'));
    });

    // Rate is editable on purchase (supplier sets the price)
    rateInput.addEventListener('input', () => calcLine(row));
...
document.getElementById('purchaseForm').addEventListener('submit', function (e) {
    ...
    rows.forEach(row => {
        ...
        const qty = parseInt(row.querySelector('.qty-input').value) || 0;
        const rate = parseFloat(row.querySelector('.rate-input').value) || 0;
        if (!med || qty < 1 || rate <= 0) valid = false;
    });

    if (!valid) {
        e.preventDefault();
        alert('Each row must have a medicine selected, rate > 0, and quantity >= 1.');
    }
});
